...
def p_2_3_l():
    ...
    На этот раз нужно выполнить другое действие —
...
def p_2_3_t():
    ...
    # 3. Для каждого блока:
    #   • Распаковать Mn, Rn, Hn (суть распаковки мне объяснял ChatGPT через
    #          аналогию с записью 256-ричного числа
    #   • Вычислить Hn = (37 * (Mn + Rn + prev_h)) % 256.
    #   • Проверить: Hn < 100 , Hn совпадает с вычислениями , если все ок,
    #       обновить прерыдущий хэш, если нет – сохранить номер
    ...
    for i in range(count):
        b = blocks[i]
        h = b % 256  # первый разряд числа
        r = (b // 256) % 256  # второй разряд числа, типа десятков
        m = b // (256**2)  # все остальное
        x = (37 * (m + r + p_h)) % 256
